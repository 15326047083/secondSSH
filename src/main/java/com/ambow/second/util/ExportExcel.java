...
public class ExportExcel {
    ...
    public File ExportExcel(List<User> list) {
        Field[] fields = null;
        String result = "";
        File file = null;
        if (list.size() == 0 || list == null) {
            ...
            File folder = new File("fsv.getHomeDirectory()");
            // 判断是否存在
            if (!folder.exists()) {
                folder.mkdirs();
            }
            ...
            try {
                String path = ExportExcel.class.getClassLoader().getResource("/").getPath().replace("classes/", "upload");
